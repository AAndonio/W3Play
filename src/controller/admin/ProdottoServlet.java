...
public class ProdottoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(action.equals("directUltimiArrivi")) {
            Prodotto product = cercaProdotto(idProdotto,(ArrayList<Prodotto>) request.getSession().getAttribute("ultimiArrivi"));
            request.getSession().setAttribute("prodotto", product);
        }
        else if(action.equals("directPiuVenduti")) {
            Prodotto product = cercaProdotto(idProdotto,(ArrayList<Prodotto>) request.getSession().getAttribute("piuVenduti"));
            request.getSession().setAttribute("prodotto", product);
        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(action.equals("addObject")) {
            ...
            if(nome.matches("[0-9a-zA-Z/:///-/' ]*$") && produttore.matches("[0-9a-zA-Z /:///-/']*$") && piattaforma.matches("[0-9a-zA-Z/:///-/' ]*$")
                    && genere.matches("[A-Za-z /']*$") &&descrizione.matches("[0-9a-zA-Z /:///-/']*$") && prezzo.matches("[0-9/.]*$")
